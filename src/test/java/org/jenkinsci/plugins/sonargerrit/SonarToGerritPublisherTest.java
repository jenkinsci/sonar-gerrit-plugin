...
public class SonarToGerritPublisherTest {
    ...
    public void testFilterByPredicates() throws IOException, InterruptedException, URISyntaxException {
        ...
        // severity predicate
        Iterable<Issue> issues = new SonarToGerritPublisher("", null, Severity.CRITICAL.name(), true, false, "", "", "", false, "", "", true, Severity.CRITICAL.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(2, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MAJOR.name(), true, false, "", "", "", false, "", "", true, Severity.MAJOR.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(12, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false, "", "", "", false, "", "", true, Severity.INFO.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(19, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MINOR.name(), true, false, "", "", "", false, "", "", true, Severity.MINOR.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(18, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.BLOCKER.name(), true, false, "", "", "", false, "", "", true, Severity.BLOCKER.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        // new issues only predicate
        issues = new SonarToGerritPublisher("", null, Severity.CRITICAL.name(), true, true, "", "", "", false, "", "", true, Severity.CRITICAL.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(0, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MAJOR.name(), true, true, "", "", "", false, "", "", true, Severity.MAJOR.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, true, "", "", "", false, "", "", true, Severity.INFO.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MINOR.name(), true, true, "", "", "", false, "", "", true, Severity.MINOR.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.BLOCKER.name(), true, true, "", "", "", false, "", "", true, Severity.BLOCKER.name(), "", "0", "0", "", "").filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(0, Sets.newHashSet(issues).size());
    }
    ...
    public void testGenerateRealNameMap() throws InterruptedException, IOException, URISyntaxException {
        ...
        SonarToGerritPublisher sonarToGerritPublisher = new SonarToGerritPublisher("", Arrays.asList(config1, config2), Severity.INFO.name(), true, false, "", "", "",  false, "", "",true, Severity.INFO.name(), "", "0", "0", "", "");
    ...
    public void testFilterIssuesByChangedLines() throws InterruptedException, IOException, URISyntaxException, RestApiException {
        ...
        SonarToGerritPublisher builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false, "", "", "", false, "", "", true, Severity.INFO.name(), "", "0", "0", "", "");
    ...
    public void getReviewResultTest() throws InterruptedException, IOException, URISyntaxException, RestApiException {
        ...
        SonarToGerritPublisher builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false,
                "No Issues Header", "Some Issues Header", "Issue Comment", false, "", "", true, Severity.INFO.name(), "Test", "+1", "-1", "NONE", "OWNER");
        ...
        builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false,
                "No Issues Header", "Some Issues Header", "Issue Comment", false, "", "", false, Severity.INFO.name(), "Test", "1", "-1", null, null);
        ...
        builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false,
                "No Issues Header", "Some Issues Header", "Issue Comment", false, "", "", true, Severity.INFO.name(), "Test", "0", "0", null, null);
        reviewResult = builder.getReviewResult(finalIssues);
        Assert.assertEquals("Some Issues Header", reviewResult.message);
        Assert.assertEquals(1, reviewResult.comments.size());
        Assert.assertEquals(1, reviewResult.labels.size());
        ...
        builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false,
                "No Issues Header", "Some Issues Header", "Issue Comment", false, "", "", true, Severity.INFO.name(), "Test", "1test", "-1test", "NONE", "ALL");
        reviewResult = builder.getReviewResult(finalIssues);
        Assert.assertEquals("Some Issues Header", reviewResult.message);
        Assert.assertEquals(1, reviewResult.comments.size());
        Assert.assertEquals(1, reviewResult.labels.size());
        ...
        builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false,
                "No Issues Header", "Some Issues Header", "Issue Comment", false, "", "", true, Severity.INFO.name(), "Test", "1", "-1", null, null);
        ...
        builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false,
                "No Issues Header", "Some Issues Header", "Issue Comment", false, "", "", true, Severity.INFO.name(), "Test", "1", "-1", "OWNER_REVIEWERS", "ALL");
        finalIssues = LinkedListMultimap.create();
        reviewResult = builder.getReviewResult(finalIssues);
        Assert.assertEquals("No Issues Header", reviewResult.message);
        Assert.assertEquals(0, reviewResult.comments.size());
