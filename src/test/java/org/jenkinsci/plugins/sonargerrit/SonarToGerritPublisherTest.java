...
public class SonarToGerritPublisherTest {
    ...
    public void testFilterByPredicates() throws IOException, InterruptedException, URISyntaxException {
        ...
        // severity predicate
        Iterable<Issue> issues = new SonarToGerritPublisher("", null, Severity.CRITICAL.name(), true, false, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(2, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MAJOR.name(), true, false, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(12, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(19, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MINOR.name(), true, false, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(18, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.BLOCKER.name(), true, false, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        // new issues only predicate
        issues = new SonarToGerritPublisher("", null, Severity.CRITICAL.name(), true, true, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(0, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MAJOR.name(), true, true, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, true, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.MINOR.name(), true, true, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(1, Sets.newHashSet(issues).size());

        issues = new SonarToGerritPublisher("", null, Severity.BLOCKER.name(), true, true, "", "", "", false, "", "", true, "", "0", "0", "", "", true).filterIssuesByPredicates(report.getIssues());
        Assert.assertEquals(0, Sets.newHashSet(issues).size());
    }
    ...
    public void testGenerateRealNameMap() throws InterruptedException, IOException, URISyntaxException {
        ...
        SonarToGerritPublisher sonarToGerritPublisher = new SonarToGerritPublisher("", Arrays.asList(config1, config2), Severity.INFO.name(), true, false, "", "", "",  false, "", "",true, "", "0", "0", "", "", true);
    ...
    public void testFilterIssuesByChangedLines() throws InterruptedException, IOException, URISyntaxException, RestApiException {
        ...
        SonarToGerritPublisher builder = new SonarToGerritPublisher("", null, Severity.INFO.name(), true, false, "", "", "", false, "", "", true, "", "0", "0", "", "", true);
